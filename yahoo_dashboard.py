...
def league_players_map(q):
    ...
    display_position and editorial_team_abbr per player — the original
    version of this map only kept the name, which is why the draft board's
    position/team columns were rendering blank.
    """
    m = {}
    try:
...
def fetch_history(q, gkeys, current_season, start_year):
    ...
    return champions, season_standings, all_time, rivalries, name_by_id
...
def build_model():
    ...
    for tx in txs:
        ...
        for p in players:
            ...
            for td in _as_list(_dig(p, "transaction", "transaction_data") or _dig(p, "transaction_data") or p.get("transaction_data")):
                ...
                activity.append({"date": "", "team": tm, "action": label, "player": name})
    ...
    # history
    champions, season_standings, all_time, rivalries, name_by_id = fetch_history(q, gkeys, season, HISTORY_START_YEAR)
    hist = {"champions": champions, "season_standings": season_standings,
            "all_time": sorted(all_time.values(), key=lambda e: (-e["win_pct"], -e["pf"])), "rivalries": []}
...
def render(model):
    panels = [
        ("standings", "Standings", standings_table(model['standings'])),
        ...
        ("draftgrades", "Draft Report Card", render_draft_grades(model.get('draft_grades'))),
